...
\subsection{Device Ready}
Come si può già notare nell'esempio \ref{fig:AngularJS-Singleton} prima che qualsiasi operazione venga eseguita si attende l'evento \emph{deviceReady}. Analogamente come \emph{document.ready()} in Javascript per le pagine web, questo evento serve a capire quando il dispositivo ha caricato tutti i dati relativi all'avvio dell'applicazione. Se si eseguissero delle operazioni prima di questo evento si correrebbe il rischio di riscontare errori o comportamenti inaspettati da parte dell'applicazione.
